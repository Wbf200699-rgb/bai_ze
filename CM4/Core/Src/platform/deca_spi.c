               ...
               const uint8_t  *bodyBuffer)
{
    ...
    HAL_SPI_Transmit(&hspi6, (uint8_t *)headerBuffer, headerLength, HAL_MAX_DELAY); /* Send header in polling mode */

    if(bodyLength != 0)
        HAL_SPI_Transmit(&hspi6, (uint8_t *)bodyBuffer,   bodyLength, HAL_MAX_DELAY);     /* Send data in polling mode */
                ...
                uint8_t       crc8)
{
    ...
    HAL_SPI_Transmit(&hspi6, (uint8_t *)headerBuffer, headerLength, 10);    /* Send header in polling mode */
    HAL_SPI_Transmit(&hspi6, (uint8_t *)bodyBuffer, bodyLength, 10);        /* Send data in polling mode */
    HAL_SPI_Transmit(&hspi6, (uint8_t *)&crc8, 1, 10);      /* Send data in polling mode */
                ...
                uint8_t   *readBuffer)
{
    ...
    {
        HAL_SPI_Transmit(&hspi6, (uint8_t*)&headerBuffer[i], 1, 100); //No timeout
    }
    ...
    while(readlength-- > 0)
    {
        ...
        hspi6.Instance->TXDR = 0; /* set output to 0 (MOSI), this is necessary for
